...
def lectreport():
    cur=mysql.connection.cursor()
    cur.execute('select user_id from enroll where course_id=%s:',)
    return ''
...
def getallqstns(level):
    if level:
        ...
        #cur = mysql.connection.cursor()
        #cur.execute('select COUNT(question_id), topic_name FROM question GROUP BY topic_name;')
        print level, cour, toc
...
def lechome2():
    return render_template('lecturer-again.html')
...
def coursestudents(cid):
    ...
    print students, 'here'
    return render_template('lecturer-students.html', students = students)
...
def coursetopics(cid):
    ...
    for row in cur.fetchall():
        topics.append(row[0])
    print topics, 'here'
    return render_template('lect-topic.html', topics = topics)
