...
def getallqstns(level):
    if level:
        ...
        cur = mysql.connection.cursor()
        num =cur.execute('select COUNT(question_id) FROM question join levels where question.topic_name=%s and levels.levels_num=%s GROUP BY topic_name;', (toc, int(level)))
        print level, cour, toc, num
        return render_template('tasknew.html',)
    
    
        return render_template('tasknew.html', question=question, hint=hint, testcase=test_cases)
def task():
    if request.method =="POST":
